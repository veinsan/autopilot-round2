...
| Workbench | Exception nyata dibuat, di-claim/acknowledge, dan di-resolve manusia; recurring risk reopen bukan duplikat | §6.3 test #1 (create); §4.R2.4 MVP-1 assertion #10 (reopen setelah resolve, `resolved_at` kembali null); **aksi manusia nyata lewat UI Command Center Workbench — Passed 2026-08-08**: claim → resolve satu case sungguhan, bukan lewat SQL | **Terpenuhi.** Catatan UX: label aksi (`Record review` = `acknowledge`/`in_review`, `Await update` = `awaiting_external_update`) dan keenam resolution code tidak menjelaskan diri sendiri, dan sempat membingungkan operator saat test. Murni masalah presentasi — enum-nya divalidasi ketat di `app/schemas/hr.py`. Polish ditunda sampai sesudah demo. |
...
| G-04 | Runtime config | Published workflow UUID mapping | Done | **Passed** | 2026-08-08: satu run Command Center berhasil menembus Auto — `command_runs.auto_run_id` **terisi**, tanpa `error_code`. Itu bukti definitif yang selama ini ditunggu; blocker `AUTO_TRANSPORT_ERROR` yang tercatat di handoff sudah tidak berlaku. Validitas `AUTO_HR_WORKFLOW_ID` terbukti lewat eksekusi, bukan lewat API baca (yang masih down — §9.4). |
| UI | Command Center | Policy Studio, Workbench, Dashboard, Data Manager | Done | **Passed** | Core build/login passed, dan aksi manusia nyata di Workbench (claim → resolve lewat UI, bukan SQL) terverifikasi 2026-08-08 — menutup gate Workbench §9. Hutang UX tercatat: label aksi case dan resolution code perlu ditulis ulang agar menjelaskan akibatnya; presentasi saja, tidak menyentuh enum. Ditunda sesudah demo. |
...
| O.1 | ORCH-01 | Parallel fan-out/merge/handoff | Done | **Passed (essential)** + **handoff re-verified pasca-rewrite OP-04** | **Re-verifikasi 2026-08-08 sesudah rewrite lima step OP-04:** satu run dari Command Center menembus Auto (`auto_run_id` terisi) dan OP-04 hasil rewrite benar-benar menulis case dengan identity kanonik baru. Ini penting karena bukti O.1 di bawah berasal dari sebelum rewrite, sementara rewrite mengubah kontrak masuk OP-04 secara mendasar (`finding.domain` caller diabaikan total, reason code divalidasi dua registry, case dikelompokkan lewat canonical identity). Seam ORCH → OP-04 pernah gagal diam-diam sebelumnya — workflow hijau tapi nol case — jadi sambungan ini sengaja diuji ulang. Bukti O.1 asli: run 2026-08-08 14:48:31–14:52:29 dari Auto test form, `EMP7032`. Fan-out paralel terbukti dari Activity Timeline: OP-02/03/05/06/07 mulai 14:48:48–14:48:52 (rentang 4 detik) dan berjalan bersamaan ±35 detik — mustahil pada eksekusi serial. Rantai keputusan utuh: Routing Logic completed, `Condition: No Escalation Needed` false, `Condition: Needs Escalation` true, `Call OP-04 Escalation Agent` completed. Merge menghasilkan `COMPLIANCE_LEGAL_BREACH` + `COMPLIANCE_DEADLINE_AT_RISK` (OP-05) dengan `policy_version_ids` nyata; OP-03/OP-06/OP-07 clear. Test #2–#5 sengaja tidak dijalankan — alasan dan bukti penggantinya di §9.4. Kontrak input: lima key dari `app/routers/hr.py`; `execution_id` dan `trigger_source` diturunkan sendiri; stage 1 tidak membaca policy. |
