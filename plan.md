...
| F-04 | Source data and policy seed | — | — | Completed foundation; 11 tables, 22 policy rows, and active versioned baseline | Done |
...
| A-01 | Capture Auto configuration | Store workflow IDs, active org, and API key in local `.env` | Review non-secret key names and config loader | Person B can build with fake values first | Not started |
...
| A-04 | Replace frontend demo pages | Dashboard, Workbench, Policy Studio, and Data Manager now use live FastAPI endpoints; simulator/twin visualization remains | Publish stable API response examples | Parallel after D-01; swap mocks for live client later | In progress |
| B-01 | Build server-side Supabase data layer | — | Server-only PostgREST repository, sanitized query boundary, deterministic evaluator | Independent of Auto credentials | In progress |
| B-02 | Build FastAPI HR APIs and demo RBAC | — | Dashboard, cases, policy/simulation, Data Manager, Insights, run API and four-role/manager checks | Uses fixed contract from D-01, not live Auto | In progress |
| B-03 | Build Auto SSE client and reconciler | Provide run IDs and live smoke-test access | Configurable `execute/stream` relay persists sanitized events and supports cancellation; reconciler remains | Can be mock-tested until A-01 is available | In progress |
| B-04 | Automated quality and privacy checks | Supply live Auto test cases | Deterministic evaluator and SSE sanitization tests added (5 passed); RBAC/remote coverage remains | Parallel; final E2E needs A-02/A-03 | In progress |
| I-01 | Connect frontend to live FastAPI | Dashboard, Workbench, Policy Studio, and Data Manager are now API-backed | Stabilize APIs and fix integration defects | First major integration point; policy migration still required | In progress |
