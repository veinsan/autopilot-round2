...
| B-02 | Build FastAPI HR APIs and demo RBAC | — | Role/capability-scoped Dashboard, standard/confidential cases, atomic policy lifecycle, simulator, Data Manager, reproducible Insights, operational twin, and run APIs | Uses fixed contract from D-01; Manager remains direct-report scoped and confidential access never upgrades standard access | Done (code) |
...
| B-04 | Automated quality and privacy checks | Supply live Auto test cases | Expanded evaluator, policy threshold, positive-allowlist privacy, Manager RBAC, parser/idempotency, operational twin, and reconciliation tests | Full backend suite passes in Docker: 123 tests; final live Auto E2E remains under I-02/Q-01 | Done (offline) |
| I-01 | Connect frontend to live FastAPI | Dashboard, Workbench, Policy Studio, and Data Manager are now API-backed | Candidate simulation retains `findings_by_code` compatibility while adding active/candidate delta; stabilize remaining integrations | Additive policy/run migration still must be applied before live use | In progress |
